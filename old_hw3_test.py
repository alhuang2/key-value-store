...
networkName = "ap-distributed-system_vpcbr" # the name of the network you created
...
def checkKey(ipPort, key, payload):
    ...
    foo = requests.get( 'http://%s/keyValue-store/search/%s'%(str(ipPort), key), data={'payload': json.dumps(payload)} )
    print(foo.json())
    return foo
